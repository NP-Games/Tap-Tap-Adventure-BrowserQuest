...
//                        if(curMinutes !== self.lastWorldChatMinutes){
                          if((new Date()).getTime() > databaseHandler.chatBanEndTime)
                              self.server.pushBroadcast(new Messages.Chat(self, msg));
                          else
                              self.send([Types.Messages.NOTIFY, "I don't know."]);
                        ...
                        if(mob.type !== "player"){
                            ...
                            if(mob.hitPoints <= 0){
                                /*if(mob.kind === Types.Entities.RAT){
                                    ...
                                    //Handle achivements/quests
                                } else if(mob.kind === Types.Entities.SKELETON){
                                    //Handle achievements or quests.
                                }*/
                            }
                            ...
                            if(mob.hitPoints <= 0){
                                ...
                                self.server.pushBroadcast(new Messages.Chat(self, "/1 " + self.name + "Dealt " + mob.name + " PK"));
                            }
                        }
                    }
                }
...
/*                            var dice = Utils.randomInt(0, 19);
    ...
    resetTimeout: function() {
        ...
        this.disconnectTimeout = setTimeout(this.timeout.bind(this), 1000 * 60 * 2); // 15 min.
    },
